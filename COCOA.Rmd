---
title: "Salmonella inactivation on cocoa beans (No moisture)"
author: "Runan"
...
# Prepare subsets of data by roasting temperature
```{r}
...
# Time temp interaction model
```{r}
tt <- lm(logreduction~ time + Temp + Temp:time,LMF_Sal)
par(mfrow=c(2,2))
plot(tt)
summary(tt)
## Overall, the slope and the intecept of the interaction term is significant
```

# Non-linear regression, T=100
...
wb100.bic <-BIC(wb100)
summary(wb100.bic)
## Weibull regression is a better model when T=100
```
...
wb120.bic <-BIC(wb120)
summary(wb120.bic)
## The fitness of the two models are similar when T=120
```
...
wb140.bic <-BIC(wb140)
summary(wb140.bic)
## Weibull model is better when T=140
```
...
p.both100 <- ggplot(T100,aes(x=time, y=-logreduction) )+
  ...
  geom_smooth(method=lm, se=FALSE, formula = y~0+x, 
              fullrange=TRUE, colour='black',linetype="dashed") +
  geom_smooth(method=nls, data=T100,
              method.args = list(formula = y~-(x/a)^b, start = list(a=18.6, b=0.62)),
              se=FALSE, colour='black') +
  ylab("Salmonella reduction \n(Log10CFU/bean)") + xlab("Roasting time (min)") 
...
# Combine all plots
```{r}
p.both.all <- multiplot(p.both100, p.both120, p.both110, p.both130, p.both115,p.both140, cols=3)
```

# Validation 
```{r}
validation <- read_excel("8_Cocoa/paper/LMF-Sal.xlsx", sheet = "validation")

```
# run one-way anova
```{r}
oneway.test(validation$Inactivation~validation$Serotype)
anov<- aov(validation$Inactivation~validation$Serotype)  
TukeyHSD(anov)
```
the p-value was larger tha 0.05 so all groups are similar

# make a boxplot
```{r}
p.vd <- ggplot(validation,aes(x=Serotype,y=Inactivation)) +
  geom_boxplot(size=1)+
  theme_bw() +
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "black"), panel.border = element_rect (colour = "black")) +
  theme(plot.margin = unit(c(1,1,1,1),"cm"))+
  theme(axis.text.x = element_text(size = 12, vjust = 1, hjust =1, color = "black", family = "Times New Roman", angle =30))+
  theme(axis.text.y = element_text(size = 14, vjust =0.5, hjust =0.5, color = "black", family = "Times New Roman"))+
  theme(axis.title.x = element_text(size = 14, vjust = -2, hjust = 0.5,color = "black", family = "Times New Roman")) +
  theme(axis.title.y = element_text(size = 14, vjust = 4, hjust = 0.5,color = "black", family = "Times New Roman")) +
